 ...
 */
public class FlowAdapter extends ViewGroup {
	...
	public FlowAdapter(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
		// TODO Auto-generated constructor stub
	}

	public FlowAdapter(Context context, AttributeSet attrs, int defStyle) {
	...
	public void setData(Context mContext, List<String> asList, List<String> tags_c) {
		if (asList == null) {
			return;
		}
		if (getChildCount() > 0) {
		...
		for (String value : asList) {
			...
			if(tags_c.contains(value)){view.setBackgroundResource(R.drawable.check_bg);
			view.setTextColor(getResources().getColor(R.color.drop_down_selected));}
			addView(view, lp);
		}

		if (onItemClickedListener != null) {
			for (int i = 0; i < getChildCount(); i++) {
				getChildAt(i).setOnClickListener(onItemClickedListener);
			}
		}
	}
